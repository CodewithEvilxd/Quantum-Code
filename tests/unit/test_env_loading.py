...
class TestEnvLoading:
    ...
    def test_user_env_path(self):
        """User .env path is in ~/.quantum_code/."""
        from quantum_code.settings import get_user_env_path

        expected = Path.home() / ".quantum_code" / ".env"
...
class TestUserConfigPath:
    ...
    def test_user_config_path(self):
        """User config path is in ~/.quantum_code/."""
        from quantum_code.models.config import get_user_config_path

        expected = Path.home() / ".quantum_code" / "config.yaml"
        assert get_user_config_path() == expected

    def test_user_config_dir(self):
        """User config dir is ~/.quantum_code/."""
        from quantum_code.models.config import get_user_config_dir

        expected = Path.home() / ".quantum_code"

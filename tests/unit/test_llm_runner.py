...
from quantum_code.utils.context import set_request_context
from quantum_code.utils.llm_runner import execute_parallel, execute_single
...
async def test_execute_parallel_all_success():
    ...
    with patch("quantum_code.utils.llm_runner._litellm_client.execute", side_effect=mock_call_async):
...
async def test_execute_single_success_with_artifacts():
    ...
    with patch("quantum_code.utils.llm_runner._litellm_client.execute", new_callable=AsyncMock) as mock_call:
        with patch("quantum_code.utils.artifacts.save_tool_artifacts", new_callable=AsyncMock) as mock_save_artifacts:
...
async def test_execute_single_routes_api_model():
    ...
    from quantum_code.models.config import ModelConfig
    ...
    with (
        patch("quantum_code.utils.llm_runner._resolver.resolve") as mock_resolve,
        patch("quantum_code.utils.llm_runner._litellm_client.execute", new_callable=AsyncMock) as mock_api_call,
        patch("quantum_code.utils.llm_runner._cli_executor.execute", new_callable=AsyncMock) as mock_cli_execute,
        patch("quantum_code.utils.artifacts.save_tool_artifacts", new_callable=AsyncMock, return_value=None),
    ):
...
async def test_execute_parallel_routes_mixed_models():
    ...
    with (
        patch("quantum_code.utils.llm_runner._resolver.resolve", side_effect=mock_resolve),
        patch("quantum_code.utils.llm_runner._litellm_client.execute", new_callable=AsyncMock, return_value=api_response),
        patch("quantum_code.utils.llm_runner._cli_executor.execute", new_callable=AsyncMock, return_value=cli_response),
        patch("quantum_code.utils.artifacts.save_tool_artifacts", new_callable=AsyncMock, return_value=None),
    ):

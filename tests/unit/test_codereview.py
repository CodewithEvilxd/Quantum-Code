...
from quantum_code.schemas.base import ModelResponse, ModelResponseMetadata
...
class TestCodeReviewStepLogic:
    ...
    async def test_no_files_returns_continue(self):
        ...
        with patch("quantum_code.utils.llm_runner._litellm_client.execute") as mock_llm:
    ...
    async def test_file_limit_enforcement_exact_boundary(self):
        ...
        with (
            patch(
                "quantum_code.utils.llm_runner._litellm_client.execute",
                return_value=mock_llm_response('{"status": "no_issues_found", "message": "All good"}'),
            ) as mock_llm,
            patch("quantum_code.utils.prompts.build_expert_context", return_value="<expert context>"),
            patch("quantum_code.utils.repository.build_repository_context", return_value=None),
        ):
            result1 = await codereview_impl(
                name="Review",
                content="Start",
...
class TestCodeReviewLLMResponseParsing:
    ...
    async def test_llm_response_files_required_to_continue(self, base_params):
        ...
        with (
            patch("quantum_code.utils.llm_runner._litellm_client.execute", return_value=mock_llm_response(json_response)),
            patch("quantum_code.utils.prompts.build_expert_context", return_value="<expert context>"),
            patch("quantum_code.utils.repository.build_repository_context", return_value=None),
        ):
            # Create thread first
...
class TestCodeReviewErrorHandling:
    ...
    async def test_llm_response_invalid_json_returns_text(self, base_params):
        ...
        with (
            patch("quantum_code.utils.llm_runner._litellm_client.execute", return_value=mock_llm_response(plain_text)),
            patch("quantum_code.utils.prompts.build_expert_context", return_value="<expert context>"),
            patch("quantum_code.utils.repository.build_repository_context", return_value=None),
        ):
            result1 = await codereview_impl(**{**base_params, "step_number": 1, "thread_id": "test-thread"})
...
class TestCodeReviewContextBuilding:
    ...
    async def test_expert_context_with_files(self):
        ...
        with patch("quantum_code.utils.llm_runner._litellm_client.execute", return_value=mock_llm_response(json_response)) as mock_llm:
    ...
    async def test_expert_context_with_issues_found(self):
        ...
        with (
            patch("quantum_code.utils.llm_runner._litellm_client.execute", return_value=mock_llm_response(json_response)) as mock_llm,
            patch("quantum_code.utils.prompts.build_expert_context", return_value="<expert context>"),
            patch("quantum_code.utils.repository.build_repository_context", return_value=None),
        ):
            result1 = await codereview_impl(
...
class TestModelStatusSummary:
    ...
    def test_extract_exception_names(self):
        ...
        from quantum_code.tools.codereview import _build_model_status_summary
...
class TestConsolidationValidation:
    ...
    async def test_consolidation_warning_when_count_increases(self, caplog):
        ...
        from quantum_code.schemas.base import ModelResponseMetadata
        from quantum_code.schemas.codereview import CodeReviewModelResult
        ...
        with (
            patch("quantum_code.utils.llm_runner.execute_parallel", return_value=mock_raw_results),
            patch("quantum_code.utils.consolidation.consolidate_model_results", return_value=mock_consolidated_result),
            patch("quantum_code.utils.prompts.build_expert_context", return_value="<expert context>"),
            patch("quantum_code.utils.repository.build_repository_context", return_value=None),
            patch("quantum_code.memory.store.store_conversation_turn"),
            patch("quantum_code.settings.settings.max_codereview_response_size", 100),  # Force consolidation
        ):
            result = await codereview_impl(
                name="Test Review",
                content="Review code",

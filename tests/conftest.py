"""Pytest configuration for quantum_code tests."""
...
async def clear_conversation_store():
    ...
    from quantum_code.memory.store import _threads
...
def mock_cli_success(mocker):
    ...
    def _setup(stdout=b"", stderr=b"", returncode=0, cli_path="/usr/bin/cli"):
        ...
        mock_exec = mocker.patch("quantum_code.models.litellm_client.asyncio.create_subprocess_exec")

        # Mock which() to indicate CLI is installed
        mocker.patch("quantum_code.models.litellm_client.shutil.which", return_value=cli_path)
...
def mock_cli_failure(mocker):
    ...
    def _setup(error_type="not_found", stderr=b"Command failed", exit_code=1):
        if error_type == "not_found":
            # CLI not installed
            mocker.patch("quantum_code.models.litellm_client.shutil.which", return_value=None)
            return None, None

        elif error_type == "timeout":
            # CLI execution times out
            mock_exec = mocker.patch("quantum_code.models.litellm_client.asyncio.create_subprocess_exec")
            mocker.patch("quantum_code.models.litellm_client.shutil.which", return_value="/usr/bin/cli")

...
**Package**: https://pypi.org/project/quantum-code/
...
- Entry points: `quantum` (CLI), `quantum-server` (MCP server), `quantum-code` (MCP server alias for uvx)

**User Installation:**
```bash
pip install quantum-code
claude mcp add quantum -- uvx quantum-code
```
...
- Calls `*_impl()` functions from `quantum_code/tools/` for actual implementation
...
- User overrides: `~/.quantum_code/config.yaml` (optional)

**`quantum_code/settings.py`**: Environment-based configuration using Pydantic Settings
- API keys loaded from `.env` files (cascading: project .env > ~/.quantum_code/.env)
...
- **Implementation**: `quantum_code/utils/context.py`
...
- User overrides via `~/.quantum_code/config.yaml` (optional, merged with package defaults)
...
1. Create Pydantic schema in `quantum_code/schemas/`
2. Create `*_impl()` function in `quantum_code/tools/`
3. Add factory wrapper in `quantum_code/server.py`: `create_mcp_wrapper(Schema, impl, "Description")`
4. Add tests in `tests/unit/` and `tests/integration/`
5. Add system prompt (if needed) in `quantum_code/prompts/`

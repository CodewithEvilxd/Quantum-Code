...
# ============================================================================
# Quantum Code Installation Script
#
# Handles environment setup, dependency installation, and MCP client
# configuration for Quantum Code server.
# ============================================================================
...
get_provider_env_vars() {
    ...
    uv run python -c 'from quantum_code.models.config import PROVIDERS; print(" ".join(env_var for pc in PROVIDERS.values() for _, env_var in pc.credentials))'
}
...
update_claude_code_allowlist() {
    # Add Quantum Code tools to Claude Code allowlist if settings.json exists with permissions key
    local settings_path
...
update_mcp_config_file() {
    ...
    local server_path="$4"    # "quantum_code.server" module name
    ...
    if [[ ! -f "$config_path" ]]; then
        ...
        print_success "Added Quantum Code to $config_name config"
    else
        ...
        mv "$temp_config" "$config_path"
        rm -f "$temp_config" 2>/dev/null || true
        print_success "Updated Quantum Code in $config_name config"
    fi
...
add_mcp_config_to_claude() {
    ...
    local server_path="quantum_code.server"
...
test_installation() {
    ...
    if "$python_path" -c "from quantum_code.server import mcp" 2>/dev/null; then
        print_success "Server module loads correctly"
    else
        print_error "Server module failed to load"
        print_info "Try running: $python_path -m quantum_code.server"
        return 1
    fi
...
setup_user_config() {
    ...
    if cp quantum_code/config/config.override.example.yaml "$user_config_file" && chmod 600 "$user_config_file"; then
...
main() {
    ...
    print_header "Quantum Code Installation"

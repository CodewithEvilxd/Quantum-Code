...
uv run pytest tests/unit/ --cov=quantum_code --cov-report=html
```
...
Found a bug or have a feature request? [Open an issue](https://github.com/Codewithevilxd/quantum-code/issues/new) and include:
...
Quantum Code uses a clean, factory-based architecture:
...
**Model Configuration:**
- YAML-based model config (`quantum_code/config/config.yaml`)
- Aliases resolve to full model names (e.g., `mini` → `gpt-5-mini`)
- Runtime defaults in Settings class (`quantum_code/settings.py` via `.env` files)
...
1. **Create Pydantic schema** in `quantum_code/schemas/` (inherit from `BaseToolRequest` or `SingleToolRequest`)
2. **Create implementation function** in `quantum_code/tools/` (e.g., `my_tool_impl()`)
3. **Add factory-generated wrapper** in `quantum_code/server.py`:
   ```python
...
6. **Add system prompt** (if needed) in `quantum_code/prompts/my_tool.md`
...
LOG_LEVEL=DEBUG uv run python quantum_code/server.py
```
...
Quantum Code has comprehensive logging for development and debugging:

**MCP Tool Logging** (`quantum_code/utils/mcp_logger.py`):
...
**LLM API Logging** (`quantum_code/utils/request_logger.py`):
...
**Updating Prompts:**
- Edit files in `quantum_code/prompts/*.md`
- Changes take effect on server restart
- Test with: `RUN_E2E=1 uv run pytest tests/integration/`

**Adding New Models:**
- Edit `quantum_code/config/config.yaml`
